...
import { contextFromHeaders, withBusinessSpan } from './tracing.js';
...
const { consumerTag } = await channel.consume(auctionNotificationsQueue, (message) => {
  ...
  void (async () => {
    ...
    try {
      await withBusinessSpan('auction.closed.consume', {
        'auction.id': parsed.data.auctionId,
        'auction.slug': parsed.data.slug,
        'messaging.destination.name': auctionNotificationsQueue,
        'messaging.message.id': parsed.data.eventId,
        'messaging.operation.name': 'consume',
      }, async (span) => {
        const emitted = await deliverAuctionOutcome({
          pool,
          event: parsed.data,
          now: new Date(),
          emit: (userId, notification) => {
            emitter.to(userRoom(userId)).emit('auction:closed', notification);
            span.addEvent('auction.outcome.emitted', {
              'notification.id': notification.notificationId,
              'notification.recipient_user.id': userId,
              'notification.recipient.role': notification.recipientRole,
            });
          },
        });
        span.setAttribute('notification.delivery.count', emitted.length);
      }, contextFromHeaders(message.properties.headers ?? {}), SpanKind.CONSUMER);
      channel.ack(message);
    } catch (error) {
