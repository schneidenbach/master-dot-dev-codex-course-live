...
**Winner purchase**:
The durable payment obligation created for one auction close, owned by its winning bidder and
fixed to the winning accepted bid amount in USD.
_Avoid_: Checkout Session, cart, order

**Hosted checkout Session**:
The replaceable Mock Stripe interaction used to attempt payment for a winner purchase.
_Avoid_: Purchase, payment record

**Paid purchase**:
A winner purchase whose matching provider completion was accepted through the authenticated,
idempotent webhook boundary.
_Avoid_: Successful redirect, checkout query parameter
