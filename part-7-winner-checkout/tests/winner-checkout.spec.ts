...
test('winner declines, cancels, retries, and completes hosted checkout', async ({ page, request }) => {
  ...
  try {
    ...
    releaseRequest();
    await page.waitForURL('http://127.0.0.1:7107/checkout/**');
    ...
    await page.waitForURL(`http://localhost:5107/items/${slug}?checkout=canceled`);
    ...
    await page.getByLabel('CVC').fill('123');
    await page.getByRole('button', { name: 'Pay $501.00' }).click();
    await page.waitForURL(`http://localhost:5107/items/${slug}?checkout=success&session_id=cs_test_*`);
