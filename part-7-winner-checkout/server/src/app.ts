...
export function buildApp({
  ...
  webBaseUrl = process.env.WEB_PUBLIC_URL ?? 'http://localhost:5107',
  stripeWebhookSecret = process.env.STRIPE_WEBHOOK_SECRET ?? 'whsec_local_part_7',
}: {
  clock?: Clock;
  ...
  app.register(async (webhookApp) => {
    ...
    webhookApp.post('/api/webhooks/stripe', async (request, reply) => {
      ...
      try {
        ...
        await client.query('COMMIT');
        return { received: true, duplicate: inserted.rowCount === 0 || purchase.status === 'paid' };
      } catch (error) {
        await client.query('ROLLBACK');
        throw error;
      } finally {
