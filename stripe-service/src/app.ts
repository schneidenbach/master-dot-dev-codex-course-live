...
export function buildStripeApp(options: {
  ...
  async function deliverCompletion(session: CheckoutSession): Promise<boolean> {
    if (!session.completion) throw new Error('Completed Session is missing its event');
    if (session.completion.delivered) return true;
    try {
      session.completion.delivered = await deliverWebhook(
        session.completion.body,
        session.completion.signature,
      );
    } catch {
      session.completion.delivered = false;
    }
    return session.completion.delivered;
  }
  ...
  app.post('/v1/checkout/sessions', async (request, reply) => {
    ...
    if (existing) {
      const matches = existing.title === lineItem.price_data.product_data.name
