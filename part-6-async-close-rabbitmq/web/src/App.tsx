...
export function App() {
  ...
  return <div className="app-shell"><SiteHeader initialQuery={query} activeUser={activeUser} />{page}<footer><div className="container"><span>Auction House</span><span className="footer-note">Database-backed demo marketplace</span><UserSwitcher users={users} activeUserId={activeUserId} error={userError} onChange={switchUser} /></div></footer>{outcomeNotifications[0] && <OutcomeModal notification={outcomeNotifications[0]} queuedCount={outcomeNotifications.length - 1} onDismiss={dismissOutcomeNotification} />}</div>;
}
